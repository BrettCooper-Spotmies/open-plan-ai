-- Add server-side context for "Access Denied" paths without leaking
-- detailed messages to clients. We use RAISE NOTICE (server logs),
-- while keeping the external exception string as 'Access Denied'.
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE NOTICE '[ensure_project_chat_group_internal] Access Denied: project not found or deleted (p_project_id=%)', p_project_id;
  ...
  IF v_project_creator IS NULL THEN
    RAISE NOTICE '[ensure_project_chat_group_internal] Access Denied: cannot determine project creator (p_project_id=% org_id=%)',
      p_project_id, v_organization_id;
    RAISE EXCEPTION 'Access Denied';
  END IF;
...
BEGIN
  ...
  IF v_caller_uid IS NOT NULL
    ...
    RAISE NOTICE '[ensure_project_creator_membership] Access Denied: created_by mismatch (caller_uid=% new_created_by=%)',
      v_caller_uid, NEW.created_by;
    RAISE EXCEPTION 'Access Denied';
  END IF;
...
BEGIN
  ...
  IF NOT public.can_manage_project_members(p_project_id) THEN
    RAISE NOTICE '[force_remove_project_chat_members] Access Denied: caller cannot manage members (p_project_id=% caller_uid=%)',
